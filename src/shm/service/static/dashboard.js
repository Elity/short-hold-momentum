...
function universePanel(){
  ...
  const market=m?'<p class="source-note">行情日期 '+esc(m.date||'尚未更新')+' · 最新日报价 '+esc(m.quote_current)+' / '+esc(m.expected)+' · 可用日线连续覆盖 '+esc(m.history_complete)+' / '+esc(m.expected)+(m.complete?' · 行情完整':' · 新增仓位暂停')+(m.missing?.length?'；待补齐：'+esc(m.missing.join(', ')):'')+'。上市历史不足 260 个交易日的证券仍不具备选股资格。</p>':'';
