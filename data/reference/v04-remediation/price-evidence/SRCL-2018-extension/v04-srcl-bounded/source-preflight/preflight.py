...
O=W/'v04-srcl-bounded/source-preflight'
START,END='2007-11-12','2018-12-28'
OLD_END='2018-03-27'
manifest=json.loads((R/'data/reference/v04-remediation/manifest.json').read_text())
scope=next(x for x in json.loads((W/'v04-archive-closure-plan/next-minimum-scope.json').read_text()) if x['ticker']=='SRCL')
iex_meta=json.loads((W/'v04-srcl-bounded/SRCL-IEX-missing-day-evidence.json').read_text())
paths={'wiki':W/'v04-remediation-archive/prices/SRCL.parquet',
       'canonical':R/manifest['price_overrides']['SRCL']['path'],
       'sheepb':Path(scope['source_path']), 'iex':W/'v04-srcl-bounded/SRCL-IEX.source.csv'}
expected={'wiki':manifest['price_overrides']['SRCL']['source_archive_sha256'],
          'canonical':manifest['price_overrides']['SRCL']['sha256'],
...
cal=xcals.get_calendar('XNYS',start=START,end='2019-01-07')
...
boundary_dates=pd.to_datetime(['2017-11-07','2017-11-08','2017-11-09','2018-03-27','2018-03-28','2018-11-27','2018-11-30','2018-12-03','2018-12-27','2018-12-28'])
boundaries=[]
...
selection_path=R/'reports/v04/v04-be90df5d090ac3ff/selection.json'
...
member_signals=[d for d in schedule.intersection(dates) if 'SRCL' in membership[d]]
coverage=[]
...
audit={'status':'SOURCE_PREFLIGHT_ONLY_NOT_APPROVED_FOR_APPLICATION','network_requests':0,'repository_writes':0,
       ...
       'source_company_labels':{'wiki':manifest['price_overrides']['SRCL']['source_name'],'sheepb':list(sheep['Company Name'].unique()),
       ...
       'not_performed':['ZIP re-extraction','web request','applicable price construction','repository or strategy changes','cross-sectional rerun']}
for path in [W/'v04-source-options-next/sheepb-2021-download.json',W/'v04-source-options-next/sheepb-extraction.json',
             W/'v04-srcl-bounded/SRCL-IEX-missing-day-evidence.json',W/'v04-archive-closure-plan/next-minimum-scope.json']:
