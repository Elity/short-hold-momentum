...
(async()=>{
 ...
 await page.locator('[data-p=opening-add]').click();
 await page.locator('#p-overlay .p-row').filter({hasText:'MSFT'}).waitFor();
 assert.equal(await page.locator('[name=opening_mark]').inputValue(),'');
 await fill('symbol','GOOG');await fill('opening_quantity','1');await page.locator('[data-p=opening-add]').click();
 await page.locator('#p-overlay .p-row').filter({hasText:'GOOG'}).waitFor();
 assert.match(await page.locator('#p-overlay .p-row').filter({hasText:'GOOG'}).innerText(),/单价 \$200\.00/);
 await page.locator('[data-p=opening-remove][data-index="1"]').click();
 await page.locator('[data-p=save-account]').click();
