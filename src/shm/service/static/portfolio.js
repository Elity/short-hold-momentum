...
window.Portfolio = (() => {
  ...
  function personal(){
    ...
    const row=(h,i)=>`<tr><td><strong>${e(insLabel(h.instrument))}</strong><small>${h.instrument.kind==='option'?'期权 · 每股报价':'股票 / ETF'}</small></td><td>${e(h.quantity)}</td><td>${money(h.value)}<small>${e(h.quote_at?localTime(h.quote_at):'等待收盘价更新')}</small></td><td>${b('记录操作','row-trade',`data-index="${i}"`)}</td></tr>`;
  ...
  async function openingQuote(){
    ...
    d.opening_mark=quote.price;d.opening_source=quote.source+' · '+localTime(quote.at);
  }
  ...
  async function act(action,target){
    ...
    if(action==='opening-quote'||action==='opening-add'){
      ...
      try{
        if(action==='opening-quote'||(d.opening_type!=='option'&&!d.opening_mark))await openingQuote();
        if(action==='opening-add')d.positions.push({instrument:readInstrument({...d,type:d.opening_type}),quantity:d.opening_quantity,mark:d.opening_mark,source:d.opening_source,original_cost:d.original_cost||null,original_opened_at:d.original_opened_at||null});
        return accountModal();
